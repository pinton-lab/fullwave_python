...
class C52VTransducerMod(C52VTransducer):
    def _make_transducer_surface_map(self, nX, nY, cen, rad):
        ...
        convex_receiver_map = WholeMapReceiver(
            num_x=nX,
            num_y=nY,
        )
...
def main():
    ...
    c52v_transducer = C52VTransducerMod(
    ...
    genout = genout_reader.load_data_and_reshape(
        genout_list[0],
        num_x=c52v_transducer.num_y,
        num_y=c52v_transducer.num_x,
        num_t=wave_transmitter.nT2 - 2,
    )

    plot_wave_propagation_animation(
        receivier_whole_map=power_compress(np.transpose(genout, (0, 2, 1))),
        dpi=300,
        num_plot_image=50,
    ...
    plot_wave_propagation_with_map(
        receivier_whole_map=power_compress(np.transpose(genout, (0, 2, 1))),
        c_map=domain_organizer.constructed_domain_dict["c_map"],
        rho_map=domain_organizer.constructed_domain_dict["rho_map"],
        dpi=300,
        num_plot_image=50,
